...
{
	public class FlareMagic : MagicType
	{
		...
		public override void ExplosionEffects(Vector2 position, float intensity = 1f)
		{
			for (int n = 0; n < 6; n++)
			{
				Dust spawnedDust = Main.dust[Dust.NewDust(position, 0, 0, ModContent.DustType<FlareDust>(), (Main.rand.NextFloat() - 0.5f) * (30f * intensity), (Main.rand.NextFloat() - 0.5f) * (30f * intensity), Alpha: (255 * .75f).Round(), Scale: intensity)];
				spawnedDust.noGravity = true;
				Dust spawnedDust2 = Main.dust[Dust.NewDust(position, 0, 0, DustID.PinkFairy, (Main.rand.NextFloat() - 0.5f) * (22f * intensity), (Main.rand.NextFloat() - 0.5f) * (22f * intensity), 0, Color.Red, 0.8f * intensity)];
				spawnedDust2.noGravity = true;
			}
		}
