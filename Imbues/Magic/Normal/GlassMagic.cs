...
namespace ArcaneOdyssey.Imbues.Magic.Normal
{
	public class GlassMagic : MagicType
	{
		...
		public override void ExplosionEffects(Vector2 position, float intensity = 1f)
		{
			for (int n = 0; n < 3; n++)
			{
				Dust.NewDustDirect(position, 0, 0, DustID.Glass, (Main.rand.NextFloat() - 0.5f) * (20f * intensity), (Main.rand.NextFloat() - 0.5f) * (20f * intensity), Scale: 2f * intensity).noGravity = true;
			}
		}
