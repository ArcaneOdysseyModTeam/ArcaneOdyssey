        ...
        public override bool? UseItem(Player player)
        {
            if (player.altFunctionUse == 2 && !player.HasBuff<RisenTide>())
            {
                ...
                for(int dustCountInt = 0;dustCountInt<10;dustCountInt++){
                    Dust.NewDust(player.position+new Vector2(-20f+(sin((float)dustCountInt)),0f),3,3,DustID.DungeonWater,0f,-3f * (float)dustCountInt,255,new Color(255,255,255,255),1f);
                    Dust.NewDust(player.position+new Vector2(20f+(sin((float)dustCountInt+3.14f)),0f),3,3,DustID.DungeonWater,0f,-3f * (float)dustCountInt,255,new Color(255,255,255,255),1f);
                }
            }
            return null;
        }
